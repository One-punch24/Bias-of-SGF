...
if __name__=='__main__':
    ...
    for i in range(3):
        ...
        ax[0].plot(x_axis,np.log10(trainloss_array_sde))
        ax[0].set_title("Train Loss")
        ax[1].plot(x_axis,np.log10(testloss_array_sde))
        ax[1].set_title("Train Loss")

    plt.legend(['SDE 1','SDE 2','SDE 3'])
    plt.xlabel("log10-iter")
    plt.ylabel("log10-trainloss")
    plt.title("Train Loss")
    save_path=os.path.join(os_path_str,'Fig_8.jpg')
