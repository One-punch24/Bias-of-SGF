...
if __name__=='__main__':
    ...
    if opt.distri=='nonuniformly':
        os_path_str='SDE_'+opt.distri+'_gamma_'+str(opt.gamma)+'_non_uni_scale_'+str(opt.non_uni_scale)+'_non_uni_start_'+str(opt.non_uni_start)
        if os.path.exists(os_path_str):
            pass
        else:
            os.mkdir(os_path_str)
    else:
        os_path_str='SDE_'+opt.distri+'_gamma_'+str(opt.gamma)+'_uni_scale_'+str(opt.uni_scale)
        if os.path.exists(os_path_str):
            pass
        else:
            os.mkdir(os_path_str)
    ...
    save_path=os.path.join(os_path_str,'Fig_5.jpg')
    ...
    plt.title("Test Loss")
    save_path=os.path.join(os_path_str,'Fig_6.jpg')
    plt.savefig(save_path)
    plt.show()
